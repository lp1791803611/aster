...
<#list cfg.columns as column>
        <#if column.isAdd == '0' && column.columnName != 'id' && column.columnName != 'is_deleted'
            && column.columnName != 'gmt_modified' && column.columnName != 'modified_by'
            && column.columnName != 'gmt_create' && column.columnName != 'create_by'
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input type="text" name="${column.propertyName}"
                           ...
                           placeholder="请输入${column.columnComment}" value="" class="layui-input">
                </div>
            </div>
        </#if>
        <#if column.showType == 'textarea'>
            <div class="layui-form-item">
                <label class="layui-form-label <#if column.isRequired == '0'>required</#if> ">${column.columnComment}</label>
                <div class="layui-input-block">
                    <textarea name="${column.propertyName}" class="layui-textarea" placeholder="请输入${column.columnComment}"></textarea>
                </div>
            </div>
        </#if>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block" th:with="type=${r'${'}@dict.getDict('${column.dictType}')${r'}'}">
                    <input type="radio" name="${column.propertyName}" th:each="dict : ${r'${type}'}"
                           th:value="${r'${dict.dictValue}'}" th:title="${r'${dict.dictLabel}'}" >
                </div>
            </div>
        </#if>
...
<script th:inline="javascript">
    layui.use(['form', 'laydate'], function () {
        ...
        <#list cfg.columns as column>
            <#if column.isAdd == '0' && column.showType == 'date'>
        laydate.render({
